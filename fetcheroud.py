...
def getRelations1(uri):
    ...
    for pair in results:
        if isDBPedia(pair['r']['value']) and not 'wiki' in pair['p']['value']:
            pairs[pair['p']['value']] = pair['r']['value']
    return pairs
...
def findParallelConnections(srcUri, parUri):
    ...
    srcrels1 = getRelations1(srcUri)
    parrels1 = getRelations1(parUri)
    print("Original has %d relations\nParallel has %d triples\nComparing %d triples" % (
    len(srcrels), len(parrels), len(srcrels) * len(parrels)))
    for key in srcrels1.keys() & parrels1.keys():
        printMatch(srcUri, key, srcrels1[key], parUri, key, parrels1[key])
